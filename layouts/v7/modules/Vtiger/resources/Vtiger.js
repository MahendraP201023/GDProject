...
Vtiger.Class('Vtiger_Index_Js', {
	...
	registerEventForLoadNewMessages : function() {
	    if(app.getModuleName() == "Leads" || app.getModuleName() == "Potentials" || app.getModuleName() == "Project"){
    	    $('.listViewActions').before('<button class="showUnRearMessages btn btn-default" style="margin: 0px 3px -45px -100px;"> UnRead Messages </button>');
	...
	registerEventForInsertMeasurementDocument : function() {
		jQuery('[name="measurement_file"]').live('change', function(e){
			setTimeout(function(){ 
	        	...
	        	var reader = new FileReader();
			   	...
			   	reader.onload = function () {
			     	jQuery('#documentFileName').val(fileName);
			     	jQuery('#documentFileData').val(reader.result);
			     	jQuery('#documentFileSize').val(fileSize);
			     	jQuery('#documentFileType').val(filetype);
			     	jQuery('.uploadedFileName').text(fileName);
			   	};
            }, 500);
	    });
    },
	...
	registerEvents: function() {
        ...
        this.registerEventForInsertMeasurementDocument();
