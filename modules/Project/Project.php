...
class Project extends CRMEntity {
	...
	function insertIntoAttachment($id,$module)
	{
		...
		if($_FILES['project_file']['name'] != '' && $_FILES['project_file']['size'] > 0){
			$_FILES['project_file']['original_name'] = vtlib_purify($_REQUEST[$fileindex.'_hidden']);
			$file_saved = $this->uploadAndSaveFile($id,$module,$_FILES['project_file']);
		}
	

		$imageNameSql = 'SELECT name FROM vtiger_seattachmentsrel INNER JOIN vtiger_attachments ON
								vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid LEFT JOIN vtiger_project ON
								vtiger_project.projectid = vtiger_seattachmentsrel.crmid WHERE vtiger_seattachmentsrel.crmid = ?';
		...
		$adb->pquery('UPDATE vtiger_project SET project_file = ? WHERE projectid = ?',array($_FILES['project_file']['name'],$id));

		//This is to handle the delete image for contacts
		if($module == 'Project' && $file_saved)
		{
			if($old_attachmentid != '')
			{
