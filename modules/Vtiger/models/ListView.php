...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public function getListViewEntries($pagingModel) {
        ...
        if (!empty($this->get('searchAllFieldsValue'))) {
		...
		foreach($listViewEntries as $recordId => $record) {
	        ...
	        if($moduleName == 'Leads' || $moduleName == 'Contacts'){
    			if($recordId){
    			    ...
    			    if($whatsappNumber){
    			        global $adb;
    			        $getWhatsappCountQuery = $db->pquery("SELECT count(*) as messagecount FROM received_whatsapp_message WHERE fromNumber LIKE '%".$whatsappNumber."%'AND status = 'unread'", array());
        			    $messagecount = $db->query_result($getWhatsappCountQuery, 0, 'messagecount');
        			    if($moduleName == 'Leads'){
        			        $adb->pquery("UPDATE vtiger_leadscf SET cf_1181 = ? WHERE leadid = ?", array($messagecount, $recordId));
    			            $querymessagecount = $adb->pquery("SELECT * FROM vtiger_leadscf WHERE leadid = ?", array($recordId));
    			            $allmessagecount = $db->query_result($querymessagecount, 0, 'cf_1181');
        			        
        			    }else if($moduleName == 'Contacts'){
        			        $adb->pquery("UPDATE vtiger_contactscf SET cf_1183 = ? WHERE contactid = ?", array($messagecount, $recordId));
    			            $querymessagecount = $adb->pquery("SELECT * FROM vtiger_contactscf WHERE contactid = ?", array($recordId));
    			            $allmessagecount = $db->query_result($querymessagecount, 0, 'cf_1183');
