    ...
    public function getWhatsAppMessages(Vtiger_Request $request) {
        ...
        for($i=0;$i < $adb->num_rows($query);$i++) {
		    ...
		    if($request->get('sourcemodule') == 'Leads'){
		        $leadsQuery = $adb->pquery("SELECT * FROM vtiger_leaddetails INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid INNER JOIN vtiger_leadaddress ON vtiger_leadaddress.leadaddressid = vtiger_leaddetails.leadid WHERE vtiger_crmentity.deleted = 0 AND vtiger_leaddetails.converted = 0 AND vtiger_leadaddress.mobile LIKE '".substr($fromNumber, 2)."%'", array());
