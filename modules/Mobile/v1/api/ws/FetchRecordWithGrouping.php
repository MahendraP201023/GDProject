		...
		foreach($moduleFieldGroups as $blocklabel => $fieldgroups) {
			...
			foreach($fieldgroups as $fieldname => $fieldinfo) {
				...
				if(isset($resultRecord[$fieldname])) {
                    ...
                    global $adb;
                    $getFieldType = $adb->pquery("SELECT * FROM vtiger_ws_fieldtype WHERE uitype = ?", array($field['uitype']));
                    $fieldtype = $adb->query_result($getFieldType, 0, 'fieldtype');
                    ...
                    }else if($field['uitype'] == 1){
                    	$field['fieldtype'] = 'text';
                    }else if($field['uitype'] == 7){
                    	$field['fieldtype'] = 'number';
                    }else if($field['uitype'] == 55 || $field['uitype'] == 255){
                    	$field['fieldtype'] = 'text';
                    }
