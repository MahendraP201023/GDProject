...
class ProjectTask extends CRMEntity {
	...
	function insertIntoAttachment($id,$module)
	{
		...
		if($_FILES['projecttask_file']['name'] != '' && $_FILES['projecttask_file']['size'] > 0){
			$_FILES['projecttask_file']['original_name'] = vtlib_purify($_REQUEST[$fileindex.'_hidden']);
			$file_saved = $this->uploadAndSaveFile($id,$module,$_FILES['projecttask_file']);
		}
	

		$imageNameSql = 'SELECT name FROM vtiger_seattachmentsrel INNER JOIN vtiger_attachments ON
								vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid LEFT JOIN vtiger_projecttask ON
								vtiger_projecttask.projecttaskid = vtiger_seattachmentsrel.crmid WHERE vtiger_seattachmentsrel.crmid = ?';
		...
		$adb->pquery('UPDATE vtiger_projecttask SET projecttask_file = ? WHERE projecttaskid = ?',array($_FILES['projecttask_file']['name'],$id));

		//This is to handle the delete image for contacts
		if($module == 'ProjectTask' && $file_saved)
		{
			if($old_attachmentid != '')
			{
