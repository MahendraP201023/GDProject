...
class Leads_FollowUp_View extends Vtiger_IndexAjax_View {
			...
			if($quotesReady == 1){
				...
				if (!empty($record)) {
					...
					if ($getDataPDF && $adb->num_rows($getDataPDF) > 0) {
						...
						$pdfDirectory = '/home2/bitechnosys/incca.crm-doctor.com/QuotePdf/';
				...
				$pdfLink = "$protocol://$hostname/" . $pdfFileLink;
				$midlewareQuery = "SELECT id, middlewareurl, authtoken FROM middleware";
				$midlewareResult = $adb->pquery($midlewareQuery);
				$midlewareRow = $adb->fetchByAssoc($midlewareResult);
				$middlewareurl = $midlewareRow['middlewareurl'];
	...
	public function design2d(Vtiger_Request $request) {
		...
		if($design2d === 1){
			...
			$query2 = "SELECT * FROM vtiger_sitevisit 
			...
			$design2DQuery = $adb->pquery($query2. "AND vtiger_sitevisit.imagename != ''", array($recordId));
			...
			$get2D = $adb->pquery($sql, array($design2DId));

			$image2D = $adb->query_result($get2D, 0, 'path').'/'.$adb->query_result($get2D, 0, 'attachmentsid').'_'.$adb->query_result($get2D, 0, 'storedname');

			$protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http';
			$hostname = $_SERVER['HTTP_HOST'];
			 $image2Durl = "$protocol://$hostname/" . $image2D;
// print_r($image2Durl);exit;


			$leadsRecordModel = Vtiger_Record_Model::getInstanceById($recordId);
	...
	public function design3d(Vtiger_Request $request) {
		...
		if($design3d === 1){
			...
			$query3 = "SELECT * FROM vtiger_sitevisit 
			INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_sitevisit.sitevisitid 
			WHERE vtiger_crmentity.deleted = 0 AND vtiger_sitevisit.sitevisit_oppid = ?";

			$design3DQuery = $adb->pquery($query3. "AND vtiger_sitevisit.imagename1 != ''", array($recordId));
			...
			$image3D = $adb->query_result($result3D, 0, 'path').'/'.$adb->query_result($result3D, 0, 'attachmentsid').'_'.$adb->query_result($result3D, 0, 'storedname');
