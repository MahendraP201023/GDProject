...
class Leads_Record_Model extends Vtiger_Record_Model {
	...
	function getPotentialsFieldsForLeadConvert() {
		...
		if ($moduleModel->isActive()) {
            ...
            $complusoryFields = array('budgetexpecations');
			foreach($fieldModels as $fieldName => $fieldModel) {
            ...
            foreach($complusoryFields as $complusoryField) {
                ...
                if($fieldModel->getPermissions('readwrite') && $fieldModel->isEditable()) {
                    ...
                    //Added Mahendra
                    if($complusoryField == 'budgetexpecations'){
                    	$record = $_REQUEST['record'];
                    	if($record){
                    		$leadsRecordModel = Vtiger_Record_Model::getInstanceBYId($record);
                    		$fieldModel->set('fieldvalue', $leadsRecordModel->get('cf_1219'));
                    	}
                    }
                    //Added Mahendra
                    $potentialFields[] = $fieldModel;
