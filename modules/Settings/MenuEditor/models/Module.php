...
class Settings_MenuEditor_Module_Model extends Settings_Vtiger_Module_Model {
	...
	public static function getAllVisibleModulesMobile() {
	    global $site_URL;
        $modules = array();
        $presence = array('0', '2');
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT * FROM vtiger_app2tab 
		WHERE visible = ? and appname not In("TOOLs", "TOOLS","SETTINGS") 
		and tabid not In(?,?,?,?,?) 
		ORDER BY appname,sequence', array(1, getTabid('Visit') ? getTabid('LocationTracking') : '',
		getTabid('LocationTracking') ? getTabid('LocationTracking') : '',
		getTabid('Employees') ? getTabid('Employees') : '',
		getTabid('Companies') ? getTabid('Companies') : '',
		getTabid('Attendance') ? getTabid('Attendance') : ''));
        $count = $db->num_rows($result);
        $userPrivModel = Users_Privileges_Model::getCurrentUserPrivilegesModel();
        if ($count > 0) {
            for ($i = 0; $i < $count; $i++) {
                ...
                $sequence = $db->query_result($result, $i, 'sequence');
                $appname = vtranslate($db->query_result($result, $i, 'appname'), 'Vtiger');
                $moduleModel->set('app2tab_sequence', $sequence);
                $moduleModel->set('label', vtranslate($moduleModel->get('label'), $moduleModel->get('name')));
                if (($userPrivModel->isAdminUser() ||
                        $userPrivModel->hasGlobalReadPermission() ||
                        $userPrivModel->hasModulePermission($moduleModel->getId())) && in_array($moduleModel->get('presence'), $presence)) {
                    $modules[$appname][$moduleName] = $moduleModel;
		...
		$modArr = [];
		foreach($modules as $mod => $node){
			$modMetArr = [];
			$modMetArr['label'] = $mod;
			$geta = [];
			foreach($node as $nody){
			       
			    if($nody->{'name'} == 'ServiceEngineer'){
			        $fileURL = $site_URL.'mobileappIcon/Default.png';
			    }else{
			        $fileURL = $site_URL.'mobileappIcon/'.$nody->{'name'}.'.png';
			    }
